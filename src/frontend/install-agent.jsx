    ...
    <main className="install-page">
      ...
      <section className="install-steps" aria-label="Device agent installation steps">
        ...
        <article className="panel install-step">
          ...
          <div><h2>Connect to the device computer via SSH</h2><p>First enable or start SSH access in the device's own administration interface. From a computer on the same trusted network, choose one connection method:</p><p><strong>Option A — Windows PowerShell</strong></p><p>Run:</p><CopyCommand>{'ssh user@<computer_ip>'}</CopyCommand><p><strong>Option B — PuTTY</strong></p><p>Before clicking <strong>Open</strong>, select <strong>SSH</strong>, enter <code>&lt;computer_ip&gt;</code> in <strong>Host Name</strong>, and leave the port as <code>22</code>. Then sign in as <code>user</code>.</p><p className="panel__hint">Use the SSH password shown or configured in the device administration interface. Once you are signed in, continue to the modem check below.</p></div>
        </article>

        <article className="panel install-step">
          <span className="install-step__number">3</span>
          <div><h2>Verify the GPS modem</h2><p>In the SSH terminal, confirm that the modem is visible:</p><CopyCommand>{'mmcli -L'}</CopyCommand><p className="panel__hint">A modem such as <code>/Modem/0</code> should appear. The unit needs a GNSS-capable modem and an antenna connected to its GNSS port.</p></div>
        </article>
        ...
        <article className="panel install-step">
          ...
          <div><h2>Connect the platform in the local panel</h2><p>The installer asks only for a local-panel password. Open the panel in step 6, then enter this platform address and the device credentials from step 1. Use <strong>Test connection</strong> before saving.</p><CopyCommand label="Copy platform address">{platformUrl}</CopyCommand><p className="panel__hint">Enter only the base address — do not add an API path. Leading and trailing spaces are removed with a visible notice, and credentials are saved only after the test succeeds.</p></div>
        </article>
