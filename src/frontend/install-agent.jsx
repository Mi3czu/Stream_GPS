    ...
    <main className="install-page">
      ...
      <section className="install-steps" aria-label="Device agent installation steps">
        ...
        <article className="panel install-step">
          ...
          <div><h2>Connect to the device computer via SSH</h2><p>First enable or start SSH access in the device's own administration interface. From a computer on the same trusted network, choose one connection method:</p><p><strong>Windows PowerShell</strong> — run:</p><CopyCommand>{'ssh user@<computer_ip>'}</CopyCommand><p><strong>PuTTY</strong> — before clicking <strong>Open</strong>, select <strong>SSH</strong>, enter <code>&lt;computer_ip&gt;</code> in <strong>Host Name</strong>, and leave the port as <code>22</code>. Then sign in as <code>user</code>.</p><p className="panel__hint">Use the SSH password shown or configured in the device administration interface. After signing in, confirm the modem is visible before installing:</p><CopyCommand>{'mmcli -L'}</CopyCommand><p className="panel__hint">A modem such as <code>/Modem/0</code> should appear. The unit needs a GNSS-capable modem and an antenna connected to its GNSS port.</p></div>
        </article>
