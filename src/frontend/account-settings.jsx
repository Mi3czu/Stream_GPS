...
const AccountSettings = () => {
  ...
  const [chatAdminForms, setChatAdminForms] = useState({ kick: { platform_user_id: '', username: '', role: 'admin' }, twitch: { platform_user_id: '', username: '', role: 'admin' } });
  ...
  const saveChatAdmin = async (platform) => {
    try { const lookup = await axios.get(`/api/v1/chat/integrations/${platform}/users/${encodeURIComponent(chatAdminForms[platform].platform_user_id)}`, { headers }); await axios.post(`/api/v1/chat/integrations/${platform}/admins`, { platform_user_id: lookup.data.user.id, username: lookup.data.user.username, role: chatAdminForms[platform].role }, { headers }); setChatAdminForms((current) => ({ ...current, [platform]: { platform_user_id: '', username: '', role: 'admin' } })); setMessage(`Trusted ${platform} user saved.`); await loadAccount(); }
    catch (requestError) { if (!handleUnauthorized(requestError)) setError(requestError.response?.data?.message || requestError.message); }
  };
  ...
  const connectKick = async () => {
    setError(null); setMessage(null);
    try {
      const response = await axios.post('/api/v1/chat/kick/connect', {}, { headers });
      window.location.assign(response.data.authorization_url);
    } catch (requestError) { if (!handleUnauthorized(requestError)) setError(requestError.response?.data?.message || requestError.message); }
  };
  ...
  return <main className="settings-page">
    ...
    {searchParams.get('chat') === 'kick-connected' && <div className="alert alert--success">Kick account connected successfully.</div>}
    {searchParams.get('chat') === 'kick-error' && <div className="alert alert--error">Kick connection failed. Check the server configuration and try again.</div>}
    ...
    <section className="panel settings-panel"><div className="panel__header"><div><h2>Chat integrations</h2><p className="panel__hint">Optional controls for location sharing and OBS overlay visibility. Nothing is connected or enabled by default.</p></div></div><div className="device-list">{chatIntegrations.map(({ platform, configured, callback_path: callbackPath, integration }) => <article className="device-card" key={platform}><div className="device-card__top"><strong>{platform === 'kick' ? 'Kick' : 'Twitch'}</strong><span className={`status-pill ${integration?.enabled ? 'status-pill--online' : 'status-pill--offline'}`}>{integration?.enabled ? 'connected' : 'not connected'}</span></div>{integration?.channel_id ? <><p className="panel__hint">Channel: {integration.channel_name || integration.channel_id}. Account authorization is saved; chat command delivery will be enabled in the next setup step.</p><button className="button--danger" type="button" onClick={() => disconnectChat(platform)}>Disconnect and remove</button></> : configured ? <><p className="panel__hint">Connect your {platform === 'kick' ? 'Kick' : 'Twitch'} account to authorize Stream GPS for chat integration.</p><button type="button" onClick={platform === 'kick' ? connectKick : connectTwitch}>Connect {platform === 'kick' ? 'Kick' : 'Twitch'}</button></> : <p className="panel__hint">Not connected yet. Add server OAuth credentials and a public HTTPS address before enabling live chat control.</p>}<div className="chat-admins"><strong>Trusted people</strong><p className="panel__hint">Enter a platform nickname. Stream GPS resolves and saves its stable account ID automatically.</p><div className="chat-admin-form"><input value={chatAdminForms[platform].platform_user_id} onChange={(event) => setChatAdminForms((current) => ({ ...current, [platform]: { ...current[platform], platform_user_id: event.target.value } }))} placeholder="Platform nickname" /><select value={chatAdminForms[platform].role} onChange={(event) => setChatAdminForms((current) => ({ ...current, [platform]: { ...current[platform], role: event.target.value } }))}><option value="admin">Trusted admin</option><option value="moderator">Moderator</option></select><button type="button" onClick={() => saveChatAdmin(platform)}>Add person</button></div><ul>{integration?.authorized_users?.map((admin) => <li key={admin.id}><strong>{admin.username || 'Unknown user'}</strong><span>({admin.platform_user_id}) · {admin.role === 'admin' ? 'trusted admin' : 'moderator'}</span><button className="button--secondary button--small" type="button" onClick={() => removeChatAdmin(platform, admin.id)}>Remove</button></li>) || <li>No trusted people added.</li>}</ul></div><small>Callback path: <code>{callbackPath}</code></small></article>)}</div></section>
