...
const ObsOverlay = () => {
  ...
  const hudAnchor = config.hudAnchor || `${config.statsPosition === 'above-map' ? 'top' : 'bottom'}-${config.statsAlign || 'left'}`;
  const statsPanel = stats.length ? (
    <section className={`obs-overlay__stats obs-overlay__stats--${config.statsLayout || 'cards'} obs-overlay__stats--${config.statsAlign || 'left'} obs-overlay__stats--${config.statsWidth || 'natural'} obs-overlay__stats--${hudAnchor}`} style={{ '--stats-text-size': `${config.statsTextSize}px` }}>
  ...
  return (
    <main className={`obs-overlay obs-overlay--${config.textTheme}`} style={{ '--overlay-text-color': config.textColor, '--overlay-font': config.fontFamily }}><div className={`obs-overlay__content obs-overlay__content--${hudAnchor}`} style={{ '--overlay-size': `${config.mapSize}px` }}>
      {statsPanel}
