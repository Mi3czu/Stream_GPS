...
const DeviceDetails = () => {
  ...
  const [routeMode, setRouteMode] = useState(() => {
    const saved = window.localStorage.getItem(`stream-gps-route-mode:${deviceId}`);
    return saved === 'raw' ? 'legacy' : (saved || 'legacy');
  });
      ...
      try {
        ...
        source.addEventListener('position', (event) => {
          ...
          setDevice((current) => current ? { ...current, last_seen_at: payload.last_seen_at, last_latitude: payload.latitude,
            ...
            last_altitude: payload.altitude, last_incline: payload.incline, last_accuracy: payload.accuracy, last_satellites: payload.satellites,
          ...
          const next = { latitude: payload.latitude, longitude: payload.longitude, speed: payload.speed, heading: payload.heading,
            altitude: payload.altitude, incline: payload.incline, accuracy: payload.accuracy, satellites: payload.satellites, recorded_at: payload.recorded_at };
          setPositions((current) => [...current.filter((item) => item.recorded_at !== next.recorded_at), next].slice(-(routeMode === 'legacy' ? 5000 : 2500)));
        });
  ...
  return <main>
    ...
    {device && <>
      ...
      <CollapsiblePanel title="Live position and route" badge={routeInfo ? `${routeInfo.rendered_points} / ${routeInfo.source_points} points` : `${positions.length} points`} defaultOpen><div className="route-mode"><div><strong>Route rendering</strong><p className="panel__hint">Legacy keeps the existing point-by-point view. Smart route preserves turns while reducing map load.</p></div><div className="route-mode__choices"><button type="button" className={routeMode === 'legacy' ? '' : 'button--secondary'} onClick={() => changeRouteMode('legacy')}>Legacy</button><button type="button" className={routeMode === 'smart' ? '' : 'button--secondary'} onClick={() => changeRouteMode('smart')}>Smart route (beta)</button><button type="button" className={routeMode === 'rounded' ? '' : 'button--secondary'} onClick={() => changeRouteMode('rounded')}>Rounded + drift correction (experimental)</button></div></div>{routeInfo && <p className="panel__hint route-mode__info">Showing {routeInfo.rendered_points} meaningful points from {routeInfo.source_points}{routeInfo.source_sampled ? ' (source sampled for a very long range)' : ''}. {routeInfo.segments > 1 ? `${routeInfo.segments} time-separated route segments are not joined.` : ''}{routeMode === 'rounded' ? ' Corners from 20° to 180° are rounded visually with a maximum 8 m radius. Very-low-speed GPS drift is corrected only for this map.' : ''}</p>}<GpsMap positions={positions.length ? positions : (device.last_latitude === null ? [] : [{ latitude: device.last_latitude, longitude: device.last_longitude, speed: device.last_speed, recorded_at: device.last_recorded_at }])} showHistoryMarkers={routeMode === 'legacy'} roundedCorners={routeMode === 'rounded'} stationaryDriftCorrection={routeMode === 'rounded'} /></CollapsiblePanel>
