...
const DeviceDetails = () => {
  ...
  const loadDevice = useCallback(async () => {
    ...
    if (routeMode !== 'raw') { params.set('route', 'smart'); params.set('max_points', '2500'); }
    ...
    const connect = async () => {
      try {
        ...
        source.addEventListener('position', (event) => {
          ...
          setPositions((current) => [...current.filter((item) => item.recorded_at !== next.recorded_at), next].slice(-(routeMode === 'raw' ? 5000 : 2500)));
        });
  ...
  return <main>
    ...
    {device && <>
      ...
      <CollapsiblePanel title="Live position and route" badge={routeInfo ? `${routeInfo.rendered_points} / ${routeInfo.source_points} points` : `${positions.length} points`} defaultOpen><div className="route-mode"><div><strong>Route rendering</strong><p className="panel__hint">Raw keeps the existing point-by-point view. Smart route preserves turns while reducing map load.</p></div><div className="route-mode__choices"><button type="button" className={routeMode === 'raw' ? '' : 'button--secondary'} onClick={() => changeRouteMode('raw')}>Raw / legacy</button><button type="button" className={routeMode === 'smart' ? '' : 'button--secondary'} onClick={() => changeRouteMode('smart')}>Smart route (beta)</button><button type="button" className={routeMode === 'rounded' ? '' : 'button--secondary'} onClick={() => changeRouteMode('rounded')}>Rounded corners (experimental)</button></div></div>{routeInfo && <p className="panel__hint route-mode__info">Showing {routeInfo.rendered_points} meaningful points from {routeInfo.source_points}{routeInfo.source_sampled ? ' (source sampled for a very long range)' : ''}. {routeInfo.segments > 1 ? `${routeInfo.segments} time-separated route segments are not joined.` : ''}{routeMode === 'rounded' ? ' Corners above 30° are rounded visually with a maximum 2.5 m radius.' : ''}</p>}<GpsMap positions={positions.length ? positions : (device.last_latitude === null ? [] : [{ latitude: device.last_latitude, longitude: device.last_longitude, speed: device.last_speed, recorded_at: device.last_recorded_at }])} showHistoryMarkers={routeMode === 'raw'} roundedCorners={routeMode === 'rounded'} /></CollapsiblePanel>
