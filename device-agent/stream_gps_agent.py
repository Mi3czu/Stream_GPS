...
def parse_mmcli(text):
    ...
    def number(*names):
        ...
        return None
    latitude = number('modem.location.gps.latitude'); longitude = number('modem.location.gps.longitude')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def authorized(self):
        ...
        try: _, password = base64.b64decode(header[6:]).decode().split(':', 1)
        except Exception: return False
        return verify_password(load_config(), password)
    ...
    def do_POST(self):
        ...
        config = load_config(); interval = int(form.get('interval_seconds', ['2'])[0])
        if not 2 <= interval <= 300: self.respond(400, 'Interval must be between 2 and 300 seconds'); return
        config.update(api_url=form.get('api_url', [''])[0].rstrip('/'), device_id=form.get('device_id', [''])[0], interval_seconds=interval, modem_id=form.get('modem_id', ['auto'])[0])
